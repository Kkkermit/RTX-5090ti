...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setFooter({ text: `🔢 Calculator Displayed | Designed By ${Owner}`})
        ...
        col.on('collect', async (i) => {
            ...
            i.update({ embeds: [new EmbedBuilder().setFooter({ text: `🔢 Calculator Command`}).setTitle('> Calculator Interface').setTimestamp().setColor('DarkBlue').setThumbnail(`${pfp}`).setDescription(`\`\`\`\n${data || extra}\n\`\`\``)], components: [row, row1, row2, row3, row4], ephemeral: false })
        })
    }
}